...
LEDGER_TEMPLATE_PATH = os.environ.get(
    "LEDGER_TEMPLATE_PATH",
    os.path.join(APP_ROOT, "templates", "safety_ledger_template.xlsx"),
)
...
def format_chinese_date(value) -> str:
    """格式化为 2026年3月31日。"""
    if not value:
        return ""
...
def _effective_check_date(issue: SafetyIssue) -> Optional[date]:
    """台账优先使用检查日期；旧数据缺失时退回创建日期。"""
    if issue.check_date:
        return issue.check_date
    if isinstance(issue.create_time, datetime):
        return issue.create_time.date()
    if isinstance(issue.create_time, str):
        try:
            return date.fromisoformat(issue.create_time[:10])
        except ValueError:
            return None
    return None
...
def _prepare_ledger_sheet(ws, required_max_row: int):
    """保留模板表头和列宽，清理示例数据后准备写入数据行。"""
    for merged_range in list(ws.merged_cells.ranges):
        if merged_range.min_row >= 3:
            ws.unmerge_cells(str(merged_range))

    if required_max_row < ws.max_row:
        ws.delete_rows(required_max_row + 1, ws.max_row - required_max_row)

    if required_max_row > ws.max_row:
        source_row = 3
        for row in range(ws.max_row + 1, required_max_row + 1):
    ...
    for row in range(3, required_max_row + 1):
        ...
        for col in range(1, 16):
            ...
            _copy_cell_style(ws.cell(source_row, col), cell)
            cell.value = None


def _ledger_issue_category(issues: List[SafetyIssue]) -> str:
    if any((issue.severity or "") == "严重" for issue in issues):
        return "重大隐患"
    return "一般隐患"


def _ledger_status(issues: List[SafetyIssue]) -> str:
    completed_status = {"已完成", "已整改"}
    if all((issue.status or "") in completed_status for issue in issues):
        return "已整改"
    today = date.today()
    if any(issue.deadline and issue.deadline < today and (issue.status or "") not in completed_status for issue in issues):
        return "逾期未整改"
    return "正在整改中"


def _ledger_deadline_value(issues: List[SafetyIssue]) -> str:
    deadlines = [issue.deadline for issue in issues if issue.deadline]
    if not deadlines:
        return ""
    return format_chinese_date(max(deadlines))


def _build_ledger_batches(issues: List[SafetyIssue], check_content: str):
    date_groups = {}
    for issue in issues:
        check_date = _effective_check_date(issue) or date.today()
        date_groups.setdefault(check_date, []).append(issue)

    batches = []
    for batch_index, check_date in enumerate(sorted(date_groups), 1):
        project_groups = {}
        for issue in date_groups[check_date]:
            project_name = (issue.project_name or "未分类").strip() or "未分类"
            project_groups.setdefault(project_name, []).append(issue)

        rows = []
        for project_name in sorted(project_groups):
            project_issues = sorted(project_groups[project_name], key=lambda item: item.id or 0)
            issue_lines = []
            for idx, issue in enumerate(project_issues, 1):
                title = (issue.title or "").strip()
                issue_lines.append(f"{idx}.{title}")

            rows.append({
                "project_name": project_name,
                "issues_text": "\n".join(issue_lines),
                "issue_count": len(project_issues),
                "category": _ledger_issue_category(project_issues),
                "deadline": _ledger_deadline_value(project_issues),
                "status": _ledger_status(project_issues),
                "line_count": max(1, len(issue_lines)),
            })

        batches.append({
            "check_date": check_date,
            "check_count": batch_index,
...
def export_ledger(
    ...
    """
    按安全检查台账模板导出检查记录，按检查日期年份过滤。
    请求体：{ "year": "2026", "check_content": "日常检查" }
    """
    try:
        year = int(body.get("year") or datetime.now().year)
    except (TypeError, ValueError):
        raise HTTPException(status_code=400, detail="年份格式无效")

    check_content = (body.get("check_content") or "日常检查").strip() or "日常检查"

    all_issues = db.query(SafetyIssue).order_by(
        SafetyIssue.check_date.asc(),
        SafetyIssue.create_time.asc(),
        SafetyIssue.id.asc(),
    ).all()
    issues = [
        issue for issue in all_issues
        if (_effective_check_date(issue) and _effective_check_date(issue).year == year)
    ]
    if not issues:
        raise HTTPException(status_code=404, detail="没有可导出的问题")

    if not os.path.exists(LEDGER_TEMPLATE_PATH):
        raise HTTPException(status_code=500, detail="台账模板文件不存在")

    batches = _build_ledger_batches(issues, check_content)
    total_rows = sum(max(1, len(batch["rows"])) for batch in batches)
    required_max_row = max(3, total_rows + 2)

    wb = load_workbook(LEDGER_TEMPLATE_PATH)
    ws = wb.active
    _prepare_ledger_sheet(ws, required_max_row)

    ws.cell(row=1, column=1, value=f"{year}年安全检查台账")
    ...
    for batch in batches:
        ...
        ws.cell(start_row, 1, format_chinese_date(batch["check_date"]))
        ws.cell(start_row, 2, batch["check_count"])
        ws.cell(start_row, 3, batch["check_content"])

        for row_data in rows:
            ws.cell(current_row, 4, row_data["project_name"])
            ws.cell(current_row, 5, row_data["issues_text"])
            ws.cell(current_row, 6, row_data["issue_count"])
            ws.cell(current_row, 7, row_data["category"])
            ws.cell(current_row, 8, row_data["deadline"])
            ws.cell(current_row, 9, row_data["status"])

            line_count = row_data.get("line_count", 1)
            ws.row_dimensions[current_row].height = max(42, min(180, 18 * line_count + 24))
            current_row += 1

    output = io.BytesIO()
    wb.save(output)
    output.seek(0)
    filename = f"safety_ledger_{year}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
...
def _export_excel_internal(issues, check_content="日常检查"):
    ...
    check_time = format_chinese_date(issues[0].check_date) or format_chinese_date(datetime.now().date())
    ...
    for idx, issue in enumerate(issues, 1):
        ...
        deadline_str = ""
        if issue.deadline:
            deadline_str = f"整改时限：{format_chinese_date(issue.deadline)}"
...
def export_rectification_reply(
    ...
    if request.check_date and request.check_date.strip():
        try:
            query = query.filter(SafetyIssue.check_date == date.fromisoformat(request.check_date.strip()))
        except ValueError:
            raise HTTPException(status_code=400, detail="检查日期格式无效")
    ...
    formatted_date = format_chinese_date(request.reply_date)
...
def export_rectification_report(
    ...
    current_row = 1
    today = format_chinese_date(datetime.now().date())
    ...
    add_info_row("回复日期", format_chinese_date(datetime.now().date()))
