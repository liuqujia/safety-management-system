...
def run_sudo_command(command):
    ...
    try:
        ...
        sudo_cmd = f"sudo -i bash -c '{command}'"
...
def main():
    ...
    output, error = run_sudo_command(cmd)
    ...
    for cmd in commands:
        ...
        if "denied" in error.lower():
            print(f"   ❌ 权限错误")
        elif "error" in error.lower() and "chdir" not in error.lower():
            print(f"   ❌ 错误: {error[:200]}")
        else:
            print(f"   ✅ 执行成功")
    ...
    commands = [
        ...
        "cd /vol2/1000/Docker/safety-vue-simple && npm install --legacy-peer-deps 2>&1 | tail -5",
        "cd /vol2/1000/Docker/safety-vue-simple && npm run build 2>&1 | tail -5",
    ...
    for cmd in commands:
        ...
        if "denied" in error.lower():
            print(f"   ❌ 权限错误")
        elif "error" in error.lower() and "chdir" not in error.lower() and "EACCES" not in error.lower():
            print(f"   ❌ 错误: {error[:200]}")
        else:
            print(f"   ✅ 执行成功")
            if output:
    ...
    print("\n6. 检查服务状态...")
    output, error = run_sudo_command("docker ps | grep -E 'safety|vue'")
    if output:
        print("   ✅ 服务运行状态:")
        print(output)
    else:
        print("   ❌ 服务未运行")
        output, error = run_sudo_command("docker ps -a")
        print("   所有容器:")
        print(output)
